 ...
 *
 * <p>a mockup command server for a JUnit test framework
 *
 * @author pcraft
    ...
    private void seekScriptFiles(String basepath) {
        ...
        for (int i = 0, len = fileArray.length; i < len; i++) {
            ...
            if (reqFile.isDirectory()) {
                ...
                File resFile =
                        new File(reqFile.getAbsolutePath().replaceAll("\\.req\\.txt", ".res.txt"));
                ...
                System.out.println(
                        "> Loaded script hash=" + hash + " file=" + reqFile.getPath() + "");
            }
        }
    }
}
