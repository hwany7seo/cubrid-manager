 ...
 */
public class BrokerConfigEditComposite extends Composite {
    ...
    public BrokerConfigEditComposite(
            Composite parent,
            int style,
            AbstractBrokerConfigEditorPart editorPart,
            String inputName) {
        super(parent, style);
                ...
                new SelectionAdapter() {
                    public void widgetSelected(final SelectionEvent event) {
                        if (event.item == sourceCTabItem) {
                            ...
                            brokerConfig =
                                    editorPart.parseStringLineToBrokerConfig(
                                            propEditor.getDocument().get());
                            ...
                            if (rect.contains(pt)) {
                                if (i == 0) {
                                    openErrorBox(
                                            ...
                                            Messages
                                                    .cubridBrokerConfEditAnnotationDialogOpenErrorMsg);
                                    return;
                                }

                                IStructuredSelection selection =
                                        (IStructuredSelection) confTableViewer.getSelection();
                        ...
                        for (int i = 0; i < columnCount; i++) {
                            ...
                            if (rect.contains(pt)) {
                                ...
                                String msg =
                                        Messages.bind(
                                                Messages
                                                        .cubridBrokerConfEditorDeleteBrokerConfConfirm,
                                                confName);
                                ...
                                for (int j = 1; j < columnCount; j++) {
                                    confTable
                                            .getColumn(j)
                                            .setText(
                                                    Messages.cubridBrokerConfEditorBrokerTitle
                                                            + (j - 1));
                                }

                                // delete data from cubridBrokerConfig, so regenerate
                                // cubridBrokerConfListData from cubridBrokerConfig
                                editorPart
                                        .getBrokerConfPersistUtil()
                                        .deleteBrokerPropertyByBrokerName(brokerConfig, confName);
                                confListData.clear();
                                confListData.addAll(
                                        editorPart.parseBrokerConfigToCommonTableValue(
                                                brokerConfig));
    ...
    public void deletePropData() {
        ...
        if (!CommonUITool.openConfirmBox(
                Messages.bind(
                        Messages.cubridBrokerConfEditorDeleteBrokerPropConfirm,
                        valueMap.get("0")))) {
            return;
        }
     ...
     * @author fulei
     */
    public class BrokerConfTableLabelProvider extends TableLabelProvider
